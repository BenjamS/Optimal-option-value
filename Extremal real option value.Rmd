...
\begin{equation}
\frac{\partial f}{\partial \zeta}\bigr_{t = \hat{t}} = \frac{2}{s^2} \left( -\frac{k(\hat{t})}{\zeta(\hat{t})} + c(\hat{t}) \right)
\label{eq:dfdzRaw}
\end{equation}
...
Note that this implies that the left-hand side of the Euler condition (equation \ref{eq:dLd\zeta}) can be rewritten as follows.
...
\begin{equation}
g(\ln(\zeta), \tau) = \frac{\partial f}{\partial \zeta} \bigr_{t = \hat{t}} \\
g_0 = \frac{\partial f}{\partial \zeta} \bigr_{t = \hat{T}},
\end{equation}
...
\begin{equation}
\frac{\partial f}{\partial \zeta} \bigr_{t = \hat{t}} = \frac{2}{s^2} c - \frac{e^{-m \tau}}{\zeta(\hat{t})}
\end{equation}
...
\begin{equation}
\frac{\partial f}{\partial \zeta} \bigr_{t = \hat{t}} = K - \frac{e^{-m \tau}}{\zeta(\hat{t})}
\end{equation}

Integrating this with respect to $\zeta$ then gives a rudimentary expression for the extremal inferential $f^*$.

\begin{equation}
f^*(\zeta(t), t)\bigr_{t = \hat{t}} = K \zeta(\hat{t}) - e^{-m \tau} \ln(\zeta(\hat{t})) + b(\hat{t})
\end{equation}
...
\begin{equation}
y(\ln(\zeta), \tau) = \frac{1}{2} ( y_0(\ln(\zeta) + m \tau) + y_0(\ln(\zeta) - m \tau)) + \frac{1}{2 m} \int_{\ln(\zeta) - m \tau}^{\ln(\zeta) + m \tau} \frac{\partial y}{\partial \tau} \bigr_{\tau = 0} \: d \omega
\end{equation}
...
\begin{equation}
\begin{split}
\frac{1}{2 m} \int_{\ln(\zeta) - m \tau}^{\ln(\zeta) + m \tau} \frac{\partial y}{\partial \tau} \bigr_{\tau = 0} \: d \omega &= \frac{1}{2 m} \int_{\ln(\zeta) - m \tau}^{\ln(\zeta) + m \tau} \left( m \frac{\partial y}{\partial \ln(\zeta)} \bigr_{\tau = 0} + \frac{s^2}{2} \right) \: d \omega \\
...
\begin{equation}
f^*(\zeta(t), t) \bigr_{t = \hat{t}} = e^{-m \tau} \left( K e^{m \tau} \zeta(\hat{t}) - \left( \ln(\zeta(\hat{t})) + \left( m - \frac{s^2}{2} \right) \tau \right)+ b(T) \right)
\label{eq:fStar}
\end{equation}
...
\begin{equation}
f^*(\zeta(t), t) \bigr_{t = \hat{t}} = e^{-m \tau} ( E[x(T)]\bigr|_{\hat{t}} - E[\ln(x(T))]\bigr|_{\hat{t}} + \ln(K) + b(T) )
\label{eq:fStarCompact}
\end{equation}

## Sufficient condition

Equation \ref{eq:fStar} is an extremal function which may either maximize, minimize, or leave stationary the action (equation \ref{eq:action}). To determine which, he sufficient condition is
...
An expression for $\partial f / \partial t$ can be obtained by once more evaluating the Black-Scholes PDE at the necessary condition, 
...
Which is negative if $k(t) > 0$. The sufficient condition for maximum action is thus satisfied so long as $k(t) > 0$. 
