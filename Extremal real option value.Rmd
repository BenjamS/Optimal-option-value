...
\begin{equation}
A(\zeta) \bigr|_{\mathcal{L} = \mathcal{L}^*} - A(\zeta + \psi(\zeta)) \bigr|_{\mathcal{L} = \mathcal{L}^*} = -m e^{-m \tau} \int_0^T \ln(\zeta + \psi(\zeta)) - \ln(\zeta) \:dt < 0
\label{eq:sufCond}
\end{equation}

The extremal function in equation \ref{eq:fStar} therefore maximizes the action.
